...
class BufferOptimizer:
    def optimize_command_buffer(self, buffer: List[List[str]]):
        ...
        for i in range(size - 1):
            cur_idx = size - 1 - i
            cur_cmd = optimized_buffer[size - 1 - i]
            self._optimize_if_prev_command_editable(cur_cmd, cur_idx, optimized_buffer)
            self._optimize_if_prev_command_useless(cur_cmd, cur_idx, optimized_buffer)
            self._optimize_if_prev_command_mergable(cur_cmd, cur_idx, optimized_buffer)
    ...
    def _optimize_if_prev_command_editable(self, cur_cmd: List[str], cur_idx: int, command_buffer: List):
        if cur_cmd is None:
            return
        for i in range(cur_idx):
            prev_cmd = command_buffer[i]
            ...
            prev_end_addr = int(prev_cmd[1]) + int(prev_cmd[2])
            if cur_cmd_addr == (prev_end_addr - 1):
                prev_cmd[2] = str(int(prev_cmd[2]) - 1)
